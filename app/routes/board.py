...
# ex) 총 페이지수 : 27일때
# => select count(bno) 총게시글수, ceil(count(bno))/25 총페이지수
...
async def view(bno: int, req: Request, db: Session = Depends(get_db)):
...
async def find(req: Request, ftype: str, fkey: str, cpg: int, db: Session = Depends(get_db)):
    try:
        ...
        print(f' ▷▷▷ find 오류발생 : {str(ex)}')
        return RedirectResponse(url='/member/error', status_code=303)


@board_router.post('/reply', response_class=HTMLResponse)
async def reply(reply: NewReply, db: Session = Depends(get_db)):
